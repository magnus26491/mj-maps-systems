...
let serverProcess: ReturnType<typeof require('child_process/spawn')> | null = null;

run('server boots and logs "Server listening"', () => {
  ...
  return new Promise<void>((resolve, reject) => {
    ...
    const timeout = setTimeout(() => {
      serverProcess?.kill();
      reject(new Error('Server did not emit "Server listening" within 15s. Output:\n' + output));
    }, 15_000);
...
run('GET /api/v1/health returns 200', async () => {
  await sleep(500);
  let attempts = 0;
...
run('GET /api/v1/health/ready returns 503 or 200 (no DB)', async () => {
  await sleep(200);
  try {
...
run('dist/ does not contain _incubator services', () => {
...
run('dist/ does not contain legacy/', () => {
